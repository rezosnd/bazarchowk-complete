...
export default function RiderLoginScreen() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (data.user?.role?.name !== 'DELIVERY_PARTNER') {
        throw new Error('Unauthorized: You are not a delivery partner');
      }
      await SecureStore.setItemAsync('rider_token', data.accessToken);
      if (data.refreshToken) {
        await SecureStore.setItemAsync('rider_refresh_token', data.refreshToken);
      }
      router.replace('/');
    } catch (e: any) {
    ...
    >
      ...
      >
        ...
        <View style={styles.form}>
          ...
          <Text style={styles.formSubtitle}>Login to your rider account</Text>
