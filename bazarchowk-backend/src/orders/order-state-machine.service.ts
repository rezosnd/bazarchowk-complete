...
export class OrderStateMachineService {
  private readonly validTransitions: Record<OrderStatus, OrderStatus[]> = {
    ...
    READY_FOR_PICKUP: [OrderStatus.ASSIGNED_TO_RIDER, OrderStatus.CUSTOMER_PICKUP, OrderStatus.PICKED_UP],
    READY: [OrderStatus.ASSIGNED_TO_RIDER, OrderStatus.CUSTOMER_PICKUP, OrderStatus.READY_FOR_PICKUP, OrderStatus.PICKED_UP],
    ...
    DELIVERED: [],
    CUSTOMER_REFUSED: [OrderStatus.RETURNING_TO_SHOP],
  ...
  validateTransition(currentStatus: OrderStatus, newStatus: OrderStatus, role: string) {
    ...
    if (newStatus === OrderStatus.DELIVERED && role !== 'ADMIN' && role !== 'DELIVERY_PARTNER' && role !== 'RIDER' && role !== 'SUPER_ADMIN') {
      throw new BadRequestException('Only riders or admins can mark an order as delivered');
    }
  ...
  validatePaymentStatus(currentPaymentStatus: PaymentStatus, newPaymentStatus: PaymentStatus, role: string) {
    ...
    if (newPaymentStatus === PaymentStatus.PAID && role !== 'SYSTEM' && role !== 'ADMIN' && role !== 'SUPER_ADMIN') {
